...
module.exports = function(options, logger, files, cb) {
  ...
  return Promise.all(groups.map(function(src) {
    ...
    if (src.vectors.length) {
      ...
      src.vectors.forEach(function(svg) {
        svg.dest = dest(src.dirname) + '.svg';
        svg.id = path.basename(svg.src, '.svg');
    ...
    return Promise.all(tasks)
      .then(function(results) {
        ...
        results.forEach(function(data) {
          if (data.vectors) {
            out.push({
              dest: path.relative(options.cwd, dest(src.dirname) + '.svg'),
          ...
          sizes.push({
            images: Object.keys(data.result.coordinates).map(function(id) {
              ...
              entry.x = image.x;
              entry.y = image.y;
              entry.dest = dest(src.dirname) + '.css';

              return entry;
            }),
            image: {
              dest: path.relative(options.cwd, dest(src.dirname) + (ratio > 1 ? '@' + ratio + 'x' : '') + '.png'),
              path: path.relative(options.dest, dest(src.dirname)) + '.png',
