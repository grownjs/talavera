...
module.exports = (options, files) => {
    ...
    if (src.vectors.length) {
      ...
      src.vectors.forEach(svg => {
        if (fs.existsSync(svg.src)) {
          const xml = fs.readFileSync(svg.src).toString();

          if (xml && xml.includes('<svg') && xml.includes('</svg>')) {
            spriter.add(path.resolve(svg.src), null, xml);
          }
        }
      });
  ...
  })).then(() => ({ data: sources, files: output }));
};
