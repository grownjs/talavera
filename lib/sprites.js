  ...
  return Promise.all(tasks)
    .then(function(results) {
      ...
      results.forEach(function(data) {
        if (data.vectors) {
          out.push({
            dest: path.relative(options.cwd, dest + '.svg'),
        ...
        sizes.push({
          ...
          image: {
            dest: path.relative(options.cwd, dest + (ratio > 1 ? '@' + ratio + 'x' : '') + '.png'),
            path: path.relative(options.dest, dest) + '.png',
      ...
      if (sizes.length) {
        ...
        out.push({
          dest: path.relative(options.cwd, dest + '.css'),
          data: tpl({
