...
function parse(options, file) {
  ...
  const absolute = path.relative(options.public, path.join(options.cwd, file.dest));
...
module.exports = (options, groups, files) => {
  ...
  function add(result) {
    for (let file in result.files) {
      ...
      (options.sizes || [2, 3]).forEach(nth => {
        ...
        let y = suffix(result.files[file].base, nth);
  ...
  if (groups && files.length) {
    ...
    files.forEach(file => {
      const relative = file.dest.substr(prefix.length + 1);
