...
function parse(options, file) {
  let relative = path.relative(options.dest, path.join(options.cwd, file.dest));
  if (options.rename) relative = path.relative(options.dest, options.rename(relative));

  const absolute = path.relative(options.public, path.join(options.cwd, file.dest)).replace(/\.{1,2}\//g, '');

  const dir = options.folders.find(x => relative.includes(`${x}/`));
  const unprefixed = dir ? relative.substr(relative.indexOf(`${dir}/`) + dir.length + 1) : relative;
