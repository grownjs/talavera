...
module.exports = function() {
  ...
  _imageTag.getScript = function(context, className) {
    return '<script>(function(){var _=' + _code.replace(/\s+/g, ' ')
      + '_(' + (context || 'null') + ',"' + (className || '') + '")})();</script>';
  };
  ...
  function _getContent(id) {
    ...
    if (obj) {
      return read(obj.id).toString();
    }
  }

  this.opts.bundleOptions.locals[options.images.get || 'getContent'] = _getContent;
  this.opts.bundleOptions.locals[options.images.tag || 'imageTag'] = _imageTag;
