...
module.exports = function() {
  ...
  function hook(key) {
    ...
    filter(options[key].filter, function(files, cb) {
      ...
      logger.status(start, 'run', `${key} < ${options[key].filter}`, function() {
        return _modules[key](options, files)
          .then(function(result) {
            ...
            return result.files.length !== 1
              ? `${key} > ${result.files.length} files written`
              : `${key} > 1 file written`;
          });
      });
    });
  ...
  this.opts.bundleOptions.helpers.icon = function(id) {
    return '<i class="' + options.prefix + '-' + id.replace(/\//g, '-') + '"></i>';
  };
