...
module.exports = function() {
  ...
  options.cwd = path.resolve(options.cwd || this.opts.cwd || process.cwd());
  options.dest = options.dest ? path.join(options.cwd, options.dest) : this.opts.dest;
  options.public = options.public ? path.join(options.cwd, options.public) : this.opts.public;

  function save(result) {
    result.files.forEach(function(file) {
      file.type = file.src ? 'copy' : 'write';
      file.dest = path.relative(options.cwd, file.dest);
      dist(file);
    });
  }
