...
module.exports = function() {
  ...
  function getAttrs(args) {
    ...
    args.slice(1).forEach(function(attrs) {
  ...
  this.opts.bundleOptions.helpers.svgi = function(_, self) {
    var file = _.id || _.src[0].split('#')[0];
    var name = _.id || _.src[0].split('#')[1];
    ...
    return body.replace('<svg', '<svg' + getAttrs(_.src));
  };

  this.opts.bundleOptions.helpers.svg = function(_) {
    return '<svg' + getAttrs(_.src) + '><use xlink:href="#' + options.prefix + '-' + (_.id || _.src[0]) + '"></use></svg>';
  };
