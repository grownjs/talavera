  ...
  return deferred.then(result => {
    ...
    if (result.data) {
      ...
      if (result.data.files) {
        Object.keys(result.data.files).forEach(file => {
          result.files.push(result.data.files[file]);
        });
      }
    }
    return result.files;
...
module.exports = (options, handlers) => {
  ...
  function svgInline(id, ref, props, ctx) {
    ...
    const regexp = new RegExp(`\\bid=["']?${ref}["']?\\b`);
  ...
  function getIcon(props, ctx) {
    ...
    if (id.indexOf('.svg') !== -1) {
      ...
      if (ref) {
        return props.inline
          ? svgInline(chunk.dest, ref, props, ctx)
          : `<svg${ctx.attributes(props, ['src', 'inline'])}><use xlink:href="#${opts.prefix}-${ref}"></use></svg>`;
      }
