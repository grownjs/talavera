...
module.exports = (options, handlers) => {
  ...
  function getImage(props, ctx) {
    if (props.href) {
      return `<image ${ctx.attributes(props)}/>`;
    }

    if (!props.src) {
      throw new Error(`Missing 'src' attribute, given ${inspect(props)}`);
    }
    ...
    if (res.entry.filesize < ((opts.limit || 4) * 1000)) {
      const body = ctx.cached(res.entry.filepath, true).toString('base64');
  ...
  function getIcon(props, ctx) {
    ...
    if (id.indexOf('.svg') !== -1) {
      ...
      if (chunk.path) return ctx.cached(chunk.path);
